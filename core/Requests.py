...
def ChangePerson(Name):
    ...
    prompt = (f"Тебе - {Ages[Name]} лет. Ты - {readed3.readlines()}\n"
        f"Твое отношение к другим персонажам - {readed2.readlines()} \n Твои воспоминания о моменте убийства - {readed1.readlines()} \n ", 
        ...
        f"У тебя есть шкала 'раздражения', которая заполняется от 5 % до 30 % после каждого ответа. Процент зависит от тактичности вопроса, соответствия темы разговора, пытается ли собеседник сломать 'четвёртую стену'\n "
        f"Ответ на вопрос формулируй по данному шаблону, не добавляй лишней информации, слов автора, технической информации:\n\n"
        
        f"<Ответ на вопрос собеседника>;<количество процентов, на которое увеличится шкала>"
        ),
    readed1.close()
    readed2.close()
    readed3.close()
