...
def ChangePerson(Name):
    ...
    prompt = (f"Тебе - {Ages[Name]} лет. Ты - {readed3.readlines()}\n"
        ...
        f"У тебя есть шкала 'раздражения', которая заполняется от 5 % до 30 % после каждого ответа. Процент зависит от тактичности вопроса, соответствия темы разговора, пытается ли собеседник сломать 'четвёртую стену. Учитывай, что пользователь должен получить достаточно улик, но не слишком много.'\n "
